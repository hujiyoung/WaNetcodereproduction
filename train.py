...
def train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):    # begin training model
        ...
        num_bd = int(bs * rate_bd)                              # the number of backdoor example
        num_cross = int(num_bd * opt.cross_ratio)               # the number of noise example
        grid_temps = (identity_grid + opt.s * noise_grid / opt.input_height) * opt.grid_rescale
        grid_temps = torch.clamp(grid_temps, -1, 1)             # limit the numbers within [-1,1]
...
def main():
    opt = config.get_arguments().parse_args()   # get user's input
    ...
    if opt.continue_training:
        ...
        # Prepare grid
        ins = torch.rand(1, 2, opt.k, opt.k) * 2 - 1                    # generate grid of 1*2*k*k, and limit it within [-1,1]
        ins = ins / torch.mean(torch.abs(ins))                          # in the paper formula (4)
        noise_grid = (
            F.upsample(ins, size=opt.input_height, mode="bicubic", align_corners=True)
            .permute(0, 2, 3, 1)
            .to(opt.device)
        )                                                               # upsampling and switch dimension to 1*h*w*2
